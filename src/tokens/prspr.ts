...
const fetcher: SupplyFetcher = async (options = defaultFetcherOptions) => {
  ...
  const circulating = Number(await blockFrost.assetsPolicyById(PRSPR_POLICY_ID).then((resp) => resp[0].quantity));
  const total = 375000000;
  const treasury = Number(await getAmountInAddresses(blockFrost, PRSPR, [TREASURY]));
  return {
