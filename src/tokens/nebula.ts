...
const fetcher: SupplyFetcher = async (options = defaultFetcherOptions) => {
  ...
  const treasury = Number(await getAmountInAddresses(blockFrost, NEBULA, treasuryRaw)) / 1e8;
  const burn = Number(await getAmountInAddresses(blockFrost, NEBULA, burnRaw)) / 1e8;
