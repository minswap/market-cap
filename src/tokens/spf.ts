...
const fetcher: SupplyFetcher = async (options = defaultFetcherOptions) => {
  ...
  const treasury = Number(await getAmountInAddresses(blockFrost, SPF, SPF_TREASURY_ADDRESSES)) / 1e6;

  const spnVault = Number(await getAmountInAddresses(blockFrost, SPF, SPECTRUM_NETWORK_VAULT_ADDRESSES)) / 1e6;

  const unclaimed = Number(await getAmountInAddresses(blockFrost, SPF, SPF_REWARDS_ADDRESSES)) / 1e6;
